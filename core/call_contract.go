...
import (
	...
	"github.com/NetSepio/astro/contract"
...
func RegisterNodeOnChain() error {
	chainName := strings.ToLower(os.Getenv("CHAIN_NAME"))
	if chainName == "sol" {
		fmt.Printf("\n%s%s%s\n", colorYellow, "═══════════ Solana Authentication ═══════════", colorReset)
		fmt.Printf("%s• Status:%s Authenticating to Solana wallet\n", colorCyan, colorReset)
		fmt.Printf("%s• Status:%s Solana Authentication successful\n", colorGreen, colorReset)
		fmt.Printf("%s%s%s\n\n", colorYellow, "══════════════════════════════════════", colorReset)
		return nil
	}
