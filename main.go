...
import (
	...
	"github.com/NetSepio/astro/api"
	"github.com/NetSepio/astro/core"
	grpc "github.com/NetSepio/astro/gRPC"
	"github.com/NetSepio/astro/p2p"
	"github.com/NetSepio/astro/util"
	"github.com/NetSepio/astro/util/pkg/auth"
	"github.com/NetSepio/astro/util/pkg/node"
...
func main() {
	...
	// Connect to VPN
	if err := core.ConnectToVPN(); err != nil {
		log.WithFields(util.StandardFields).Errorf("Failed to connect to VPN: %v", err)
	}
	...
	<-sigChan
	log.WithFields(util.StandardFields).Info("Received termination signal, starting graceful shutdown...")

	// Disconnect VPN
	log.WithFields(util.StandardFields).Info("Disconnecting from VPN...")
	if err := core.DisconnectVPN(filepath.Join(os.Getenv("WG_CONF_DIR"), "client.conf")); err != nil {
		log.WithFields(util.StandardFields).Errorf("Error during VPN disconnection: %v", err)
	} else {
		log.WithFields(util.StandardFields).Info("VPN disconnected successfully")
	}

	// Wait for all goroutines to finish
	wg.Wait()
	log.WithFields(util.StandardFields).Info("Shutdown complete")
}
